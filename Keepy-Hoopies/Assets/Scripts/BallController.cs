...
public class BallController : MonoBehaviour {
	...
	void Update () {
        if (gameManagerScript.getGameLostState() && gameObject.activeSelf)
        {
        ...
        if (ballTrackerActive && !ballTracker.activeSelf)
        {
            ballTracker.SetActive(true);
        }
        else if (!ballTrackerActive && ballTracker.activeSelf)
        {
    ...
    public void centreBallNoGravityToggle()
    {
        sphere.velocity = Vector3.zero;
        transform.position = centreLocation;
    }
